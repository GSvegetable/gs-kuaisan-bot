...
async def scheduled_job(context: ContextTypes.DEFAULT_TYPE):
    try:
        if not context.bot_data.get('is_broadcast_enabled', False):
            return
        ...
        # 向所有已开启播报的群组发送
        active_groups = context.bot_data.get('active_groups', [])
...
async def ks_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    active = context.bot_data.setdefault('active_groups', [])
    if current_chat_id not in active:
        active.append(current_chat_id)
    context.bot_data['is_broadcast_enabled'] = True
    
    has_job = any(job.name == "kuaisan_job" for job in context.job_queue.jobs())
    if not has_job:
        context.job_queue.run_repeating(scheduled_job, interval=180, first=5, name="kuaisan_job")
    await update.message.reply_text(f"🤖 本群（{current_chat_id}）快三开奖播报已开启！")

async def ks_guanbi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_chat_permission(update, context): return
    current_chat_id = update.effective_chat.id
    active = context.bot_data.get('active_groups', [])
    if current_chat_id in active:
        active.remove(current_chat_id)
    if not active:
        context.bot_data['is_broadcast_enabled'] = False
        for job in context.job_queue.jobs():
            if job.name == "kuaisan_job":
                job.schedule_removal()
    await update.message.reply_text(f"✅ 本群快三开奖播报已关闭。")
...
def main():
    ...
    application.bot_data['active_groups'] = []
    ...
    logging.info("✅ 快三开奖机器人（多群动态版）已上线！")
